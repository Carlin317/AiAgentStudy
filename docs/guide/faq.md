...
如果你第一次接触 AI 智能体开发，建议先阅读本文，完整章节结构见 [教程目录大纲](/guide/outline)。
...
| 你的目标               | 推荐起点                                        | 推荐主线                                             | 适合人群                                   |
| ---------------------- | ----------------------------------------------- | ---------------------------------------------------- | ------------------------------------------ |
| 零基础系统入门         | [第 1-1 章](/stage1-foundation/ch01-1)        | `1-1 -> 1-2 -> 1-3 -> 2 -> 3 -> 9 -> 10~21 -> 22~26` | 对大模型、RAG、Agent 还没有完整认知的同学  |
| 尽快跑通第一个代码案例 | [第 10 章](/stage3-framework/ch10) | `10 -> 11 -> 13 -> 14 -> 15 -> 17 -> 21 -> 22`       | 已有 Python 基础，想快速进入代码实践的同学 |
| 做项目或准备面试       | [第 9 章](/stage3-framework/ch09)             | `9~26 -> 电商问数 -> 深度研搜 -> 面试题库`           | 需要“原理 + 实操 + 项目表达”一起建立的同学 |
...
- [电商问数：前言](/stage4-projects/ecommerce/)
...
- **申请步骤**：教程中有图文说明，可直接查看 [第 10 章 - LangChain 快速上手与 HelloWorld](/stage3-framework/ch10) 的「**2、大模型服务平台**」（含阿里云百炼平台、API-Key 管理入口表格）与「**4、实战：基于阿里百炼的 HelloWorld**」（含 API Key 获取、模型名、Base URL 三件套）。简要步骤如下：
...
- **申请步骤**：教程中有截图与步骤，见 [第 10 章 - LangChain 快速上手与 HelloWorld](/stage3-framework/ch10)（含「多模型共存」中 DeepSeek 的 API Key 获取与配置）。简要步骤如下：
...
- 若案例中使用的是 `openai` 库 + `base_url` 方式（例如接 DeepSeek、国内兼容 OpenAI 的厂商），通常只需在 `.env` 里配置对应厂商的 Key 和文档中的 `base_url` 即可。**接入方式与参数说明**见 [第 11 章 - Model I/O 与模型接入](/stage3-framework/ch11) 的「3.1 使用 OpenAI 兼容接口」与「3.3 接入通义千问（阿里云百炼）」。
...
- 完整章节导航：[教程目录大纲](/guide/outline)
- 术语随查：[全书术语表](/guide/glossary)
- 案例汇总：[教程案例链接汇总](/guide/case-links)
- 工具与资料：[工具导航与参考资料索引](/guide/resources)
- 面试复盘：[AI 智能体与大模型应用开发面试题库](/guide/interview)
- 实战项目：[电商问数](/stage4-projects/ecommerce/)
