...
> [第 8 章 Docker 入门与 Dify 部署常见问题](/stage2-lowcode/ch08)
...
这部分属于 Docker 通用问题，完整解释统一放在 [第 8 章 - 网络慢或镜像拉取失败](/stage2-lowcode/ch08#12、网络慢或镜像拉取失败)。本节只保留本项目启动前需要知道的操作入口。
...
`up`、`stop`、`down`、`down -v` 的区别见 [第 8 章 - 常用 Compose 命令](/stage2-lowcode/ch08#5、常用-compose-命令)。这里尤其要记住：如果只是停止本项目基础服务，不要随手执行 `docker compose down -v`，它会删除命名卷中的 MySQL、Elasticsearch、Qdrant 数据。
