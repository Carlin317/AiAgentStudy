“””
【案例 06-2】Command 对象：节点返回 Command(update=..., goto=...)，在「更新状态」的同时「指定下一跳」，
实现状态更新与路由一步完成，适合人机闭环与多智能体交接。

对应教程章节：第 24 章 - LangGraph API：节点、边与进阶 → 3、Send、Command 与 Runtime 上下文

知识点速览：
- Command(update=..., goto=...) 先按 Reducer 合并 update，再决定下一跳；与普通 dict 返回的关键区别。
- 与条件边的区别：条件边是”节点执行完后再单独路由”，Command 是”节点自己就是决策点，离场时把状态和去向一起交代”。
- 带循环的图应配合明确的终止条件与递归上限，避免流程跑飞。
“””
