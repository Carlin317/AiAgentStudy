“””
【案例 07-12】时间旅行：在带 checkpointer 的图上跑完全程，用 get_state_history 选历史快照，
update_state 改写状态后 invoke(None, new_config) 从分叉点重跑。

对应教程章节：第 25 章 - LangGraph 高级特性 → 3、时间回溯（Time-Travel）

知识点速览：
- 步骤：invoke 跑图 → get_state_history 找 checkpoint_id → update_state 改 values → invoke(None, config) 继续。
- update_state 返回的新 config 含新 checkpoint，后续应使用该 config 作为时间旅行起点。
- NotRequired 表示该键在开始时可不出现，适合分步填满的状态。
- Time-Travel 本质上是在已有 checkpoint 历史上”回到过去、再分一条新路径”。
“””
...
def main():
    ...
    # states1[2] 对应 create_character 执行后的快照，索引与 get_state_history 顺序有关
