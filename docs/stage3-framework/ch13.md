...
也就是说，这一章不是零散的 API 介绍，而是在回答一个更大的问题：**当我们真正开发一个 LLM 应用时，应该怎样把 [1-2 提示词工程基础](/stage1-foundation/ch01-2) 里学到的那些写法和原则，组织成一套可复用、可维护、可扩展的结构？**

---

## 2、调用大模型的入参类型

初学者最容易有一个误解：以为调用聊天模型时，输入永远只能是“一段字符串”。实际上，LangChain 聊天模型为了适配真实对话场景，通常支持多种输入形态。它们表面写法不同，但本质上都在表达一件事：**你希望把 [1-2 章](/stage1-foundation/ch01-2) 里讲过的角色、任务、上下文、输入和约束，以什么角色和什么结构交给模型。**

### 2.1 入参形态总览

同一次 `invoke`，左侧可以是多种类型的输入，中间由聊天模型处理，右侧典型返回值是 `AIMessage`。这也是为什么你在 [第 11 章](/stage3-framework/ch11) 会经常看到“正文一般通过 `.content` 读取”。
...
- **本地开源模型（Ollama 等）**：实例化与端点配置见 [第 12 章 Ollama 本地部署与调用](/stage3-framework/ch12)，本章的 `invoke` / `stream` / `batch` 用法同样适用。
...
这段代码的价值在于，它让模板本身变得非常稳定，而把“历史有几轮、具体内容是什么”延迟到运行时再决定。后面学 [第 16 章 记忆与对话历史](/stage3-framework/ch16) 时，你会频繁看到这种模式。
...
运行这类案例时，一定要注意**当前工作目录**，否则相对路径可能找不到文件。这一点和 [第 10 章 HelloWorld](/stage3-framework/ch10) 中的 `.env` 读取问题本质很像，都是“脚本运行位置”和“相对路径”的关系。
...
**建议下一步：** 继续学习 [第 14 章 输出解析器](/stage3-framework/ch14)，把本章的“输入组织”与下一章的“输出结构化”连起来；再配合 [第 15 章 LCEL 与链式调用](/stage3-framework/ch15)，就能形成 LangChain 中最核心的“输入 -> 模型 -> 输出 -> 链式编排”主线。如果你读到这里，仍然对“为什么要这样拆 Prompt”不够踏实，也可以回看 [1-2 提示词工程基础](/stage1-foundation/ch01-2) 中的六要素、Few-shot 和结构化组织方式部分。
