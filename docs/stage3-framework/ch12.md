...
[LangChain_Ollama.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/03-ollama/LangChain_Ollama.py ":include :type=code")
...
**建议下一步：** 先亲手完成这条最小链路：安装 Ollama → `ollama run qwen:4b` 跑通一次本地模型 → 用 [LangChain_Ollama.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/03-ollama/LangChain_Ollama.py) 在 Python 里调通本机模型。跑通之后，再进入 [第 13 章 提示词与消息模板](/stage3-framework/ch13)、[第 14 章 输出解析器](/stage3-framework/ch14)、[第 15 章 LCEL 与链式调用](/stage3-framework/ch15)，把本地模型也串进完整的 LangChain 工作流里。
