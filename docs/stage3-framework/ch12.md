...
- 学会使用 **LangChain** 里的 `ChatOllama` 调用本机 Ollama 服务，理解它与 [第 11 章 Model I/O 与模型接入](/stage3-framework/ch11) 中云端模型调用的异同。

**前置知识：** 已完成第 10、11 章的云端模型调用。本章重点不在多背命令，而在亲手确认"本地模型能被服务化调用"。先装好 Ollama，拉取一个小模型，命令行验证能回答，再接到 LangChain 的 `ChatOllama`。排障时主要检查模型名称、模型存放位置和机器资源。
...
如果本地还没有这个模型，Ollama 会先下载；下载完成后直接启动并进入交互模式。相比自己处理模型权重、推理框架、启动服务、配置推理端口，这要简单得多。

![Ollama 官方标志：羊驼线稿风格图标（品牌识别，常见于安装包与文档）](/images/12/12-1-1-1.png)

### 1.2 Ollama 解决了什么问题

前两章学的云端模型调用，优点是简单、稳定、开箱即用；但它也有明显局限：需要联网；需要 API Key；会产生调用费用；某些场景下数据不能离开本地机器或企业内网。

Ollama 对应的是另一条路：**把模型放到你自己的电脑上跑。**
...
很多人学 Ollama 时会下意识地把它当成"另一个模型平台"，其实它更像是**本地运行层**。
...
- **企业内网原型 / 隐私敏感验证**：也适合
- **对性能、稳定性、并发要求很高的正式推理服务**：要根据业务评估是否继续用 Ollama，还是切向更专业的推理部署方案

同时也要有合理预期：

- 不等于"任何模型都能在你的电脑上丝滑跑起来"
- 不等于"本地跑一定比云端便宜或更强"
- 更像一个**本地推理入口**，适合开发、实验、教学和一些轻中量应用
...
> **Ollama 适合学习本地模型、开发调试和轻量本地服务，但不是所有生产场景的唯一方案。**
...
这也是很多教程安装完很快就会讲 `OLLAMA_MODELS` 的原因——模型文件一旦大起来，很容易把系统盘吃满。
...
Windows 和 macOS 则更常见的是图形安装包。另外，LangChain 官方 ChatOllama 文档还提到：
...
而不是一开始就追求更大的模型。本项目案例里也使用了更适合本地体验的标签，例如 `qwen:4b`。
...
如果你希望把 Ollama 或模型文件安装到非默认路径，例如 D 盘、大容量盘或专门的数据盘，建议尽早规划模型目录。
...
如果要改路径，官方同样建议用 `OLLAMA_MODELS`。另外要注意：
...
如果之前已经在其他目录下载过模型，可以直接把已有模型目录迁移过去，避免重复下载。
...
很多"明明改了路径但不生效"的问题，本质是环境变量设置方式不对。
...
这条命令在真实项目里很实用，因为它不只是告诉你"模型有没有运行"，还能帮助你判断：
...
这属于进阶知识，但建议先知道。Ollama 支持通过 **Modelfile** 创建自定义模型。Modelfile 相当于：

> **Ollama 中"如何基于一个已有模型，定义系统提示、参数、模板等规则"的配置蓝图。**

本章不展开自定义模型构建细节，但至少要知道：`ollama create my-model -f Modelfile` **不是**重新训练模型，而更像是**基于已有模型做运行时封装**（系统提示、参数、模板等）。
...
安装完成后，先做两个最基础的验证：1. 命令是否可用；2. 本地服务是否真的在监听。
...
Ollama 本地 API 默认端口是 **11434**。服务正常启动后，通常会在这个端口监听。
...
想知道 Ollama 里有哪些模型，最直接的方式是去官方模型库：
...
如果官方模型库里已经推荐了更新的 Qwen tag，可以优先使用当前页面里适合自己机器配置的轻量版本。

提醒：

> **模型标签是否可用，要以当时官网模型库里的实际名称为准。**

不要死记教程里某个标签，要学会自己去模型库里查。

### 4.4 使用课程资料中的离线模型

如果课程资料或你自己的磁盘里已经有现成的 `models` 目录，可以直接通过：
...
只会在终端里 `ollama run`，还不等于能把它接到自己的项目中。
...
`ChatOllama` 是 LangChain 中用于连接本地 Ollama 聊天模型的类——本地模型版本的 Chat Model 客户端。

它在使用体验上和你前面学过的 `ChatOpenAI`、`init_chat_model(...)` 有很多共通点：
...
这和前面章节里学的"字符串 `invoke()`"几乎一样，只是端点从云端换成了本地。

### 5.4 消息列表写法：更贴近后续章节

如果希望和 [第 13 章 提示词与消息模板](/stage3-framework/ch13) 保持一致，也可以传消息列表：
...
这种写法的价值在于：后面学 Prompt、Messages、多轮历史、Agent 时，思路是连续的。

### 5.5 显式写 base_url 原因

为什么有时代码里写 `base_url="http://localhost:11434"`，有时不写？

- **不写**：默认走本机默认地址
- **写了**：更显式、更清楚，也方便以后切到远程 Ollama 或不同端口
...
在教学上也更直观，因为你能明确知道自己连的是哪个服务地址。

### 5.6 基本案例

【案例源码】`案例与源码-2-LangChain框架/03-ollama/01_LangChain_Ollama.py`

[01_LangChain_Ollama.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/03-ollama/01_LangChain_Ollama.py ":include :type=code")

这个案例是本章最核心的代码落地点，对应的是：
...
1. Ollama 跑在本地，和调用云端 API 相比，真正改变了哪些事情？
   ...
   **参考思路：** 模型权重、推理计算和数据流在本机，隐私和控制力更强，但也要承担本机资源、模型下载、性能和运维问题。

2. 如果 `ollama run` 可以回答，但 LangChain 代码调用失败，你会怎么查？

   **参考思路：** 先确认 Ollama 服务是否在 `11434` 端口运行，再查模型名是否一致、`ChatOllama` 配置是否正确、依赖版本是否安装、代码能否访问本机地址。

3. 本地模型效果不好时，你会先换模型，还是先检查任务和提示词？

   **参考思路：** 先看任务是否适合当前模型规模，Prompt 是否清楚，输入是否过长或缺上下文。小模型能力有限，但也要确认不是任务设计的问题。

4. 为什么本地部署时要关心模型大小、内存、显存和并发？

   **参考思路：** 本地机器资源有限，模型越大，占用越高，响应越慢，并发越容易卡。个人实验和多人服务的需求差距很大。
...
**建议下一步：** 先亲手完成这条最小链路：安装 Ollama → `ollama run qwen:4b` 跑通一次本地模型 → 用 [01_LangChain_Ollama.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/03-ollama/01_LangChain_Ollama.py) 在 Python 里调通本机模型。跑通之后，再进入 [第 13 章 提示词与消息模板](/stage3-framework/ch13)、[第 14 章 输出解析器](/stage3-framework/ch14)、[第 15 章 LCEL 与链式调用](/stage3-framework/ch15)，把本地模型也串进完整的 LangChain 工作流里。
