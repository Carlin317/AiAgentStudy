...
- 会运行并理解本章全部案例：**环境检查、HelloWorld、多模型共存、企业级封装、流式输出**，为后续 [Model I/O](/stage3-framework/ch11)、[Ollama 本地调用](/stage3-framework/ch12)、[提示词与消息模板](/stage3-framework/ch13)、[输出解析器](/stage3-framework/ch14) 打基础。

**前置知识：** 已读完 [第 9 章](/stage3-framework/ch09) 的架构概览。本章目标是跑通第一次模型调用。先确认依赖、模型名、API Key、Base URL，再看代码结构；成功后再比较多模型共存和工程化封装。

**官方文档与资源**：详见 [工具导航与参考资料索引 - LangChain](/guide/resources#LangChain)。
...
2. **便于迁移理解**：无论百炼、DeepSeek 还是其他兼容 OpenAI 协议的平台，本质上都绕不开 **API Key、模型名、Base URL** 这套调用逻辑。

本章虽然主要用 **阿里百炼 + DeepSeek** 举例，但你真正要学会的是"**怎么用 LangChain 接模型**"，而不是只会某一个平台。

### 1.2 Python 版本与项目环境约定

这一点直接决定后面会不会遇到兼容性问题，必须先说明。

根据 **LangChain 1.x 官方安装文档**和本项目当前依赖约定：

- **推荐版本**：Python **3.10**
- **支持范围**：Python **3.10–3.13**
- **不建议使用**：Python **3.14**

本仓库根目录的 [requirements.txt](requirements.txt) 已明确写明：项目当前推荐 **Python 3.10**，并说明 **`langchain-redis` 等依赖暂未兼容 3.14**。本章不再沿用旧资料里常见的"Python 3.8+"说法，直接按本项目约定来，后续章节更省心。
...
更细的环境准备步骤，可配合 [新手入门与常见问题](/guide/faq) 一起看。

### 1.3 运行案例前置注意事项

很多人第一次跑不通不是代码问题，而是下面两个约定没注意到。
...
```bash
python 案例与源码-2-LangChain框架/01-helloworld/03_LangChainV1.0.py
```
...
无论最终接的是阿里百炼、DeepSeek、OpenAI，还是其他兼容 OpenAI 协议的平台，绑定模型调用的始终是三项信息：
...
没有 API Key，平台不知道你是谁；没有模型名，平台不知道你要调哪个模型；没有 Base URL，请求不知道该发往哪里。本章后面的 HelloWorld 就是围绕这三件套展开。

### 2.2 常见平台一览

下面列出当前学习中比较常见的平台。不需要一开始全部注册，但至少要知道它们的角色：
...
跟着本仓库按章节学习，**最推荐的方式是直接安装项目依赖**，而不是手动一个个装包：

```bash
pip install -r requirements.txt
```

这样能和本仓库案例保持一致，后续学到 Prompt、Parser、LCEL、Memory、RAG、Agent 时也不用频繁补装依赖，避免"当前章节能跑、下一章突然缺包"的情况。
...
如果只想先跑通本章 HelloWorld，也可以安装最小依赖集合。
以本章案例为核心，建议至少安装：
...
安装完成后，建议先验证环境，提前发现"装错 Python""装进了别的虚拟环境""版本对不上"等问题。

**方法一：运行环境检查脚本**

【案例源码】环境检查脚本：`案例与源码-2-LangChain框架/01-helloworld/01_GetEnvInfo.py`

[01_GetEnvInfo.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/01-helloworld/01_GetEnvInfo.py ":include :type=code")
...
在真实项目里它的价值很大，因为很多"明明装了包却提示找不到"的问题，最后都是因为运行脚本时用的不是同一个 Python 环境。
...
无论接的是百炼还是其他平台，写代码时都离不开三件套。这里用百炼做一次完整说明。
...
如果走的是 OpenAI 兼容接法，就需要对应的兼容接口地址，例如：
...
这一节保留两种写法，不是因为新项目里都要用，而是因为现实里你一定会同时遇到两类资料：
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/02_LangChainV0.3.py`

[02_LangChainV0.3.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/01-helloworld/02_LangChainV0.3.py ":include :type=code")

这个案例的价值在三点：

- 展示了 **`ChatOpenAI` + `base_url`** 这种经典的 OpenAI 兼容接法
- 明确对比了 **硬编码 Key → 环境变量 → `.env`** 三种配置方式
- 让你看到 `invoke()` 返回的是一个对象，而不只是字符串

其中最重要的工程习惯是：**不要把 API Key 写死在代码里**。

#### 4.3.2 方式二：LangChain 1.x 推荐写法

【案例源码】`案例与源码-2-LangChain框架/01-helloworld/03_LangChainV1.0.py`

[03_LangChainV1.0.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/01-helloworld/03_LangChainV1.0.py ":include :type=code")

这个案例是当前更推荐重点掌握的写法。它的意义在于：通过 **`init_chat_model` 统一入口**，不再需要为每个模型厂商记一套不同的初始化方式，而是先记住同一套调用骨架，再通过参数切换不同模型和 provider。

### 4.4 0.3 与 1.x 写法到底差在哪

这个问题必须讲透，否则后面看旧代码会很乱。

**0.3 / 经典写法的思路**：

- 直接从具体集成包导入类，例如 `ChatOpenAI`
- 类名本身就带有"我是按哪种协议接入"的语义
- 代码直观，但不同厂商、不同类名会让项目越写越散
...
> 从零开始做新项目，优先学 **1.x 写法**；维护现有项目，能读懂 **0.3 / 经典写法** 同样重要。
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/04_LangChain_MoreV1.0.py`

[04_LangChain_MoreV1.0.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/01-helloworld/04_LangChain_MoreV1.0.py ":include :type=code")
...
这已经接近真实项目了。正式项目里很少只保留一个模型对象，而是会把多个模型按用途封装，例如：
...
如果只是做临时脚本，HelloWorld 那种"写几行代码直接调模型"的方式已经够用了。但一旦放进真实项目，就会遇到这些问题：
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/05_StandardDesc.py`

[05_StandardDesc.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/01-helloworld/05_StandardDesc.py ":include :type=code")

这个案例比前面的 HelloWorld 更接近真实项目，体现在：
...
前面的案例教你"怎么调通"，这个案例教你"怎么写得像一个真正的项目"。
...
1. 第一次 LangChain 调用失败时，你会按什么顺序排查？
   ...
   **参考思路：** 先查依赖是否安装，再查环境变量、Base URL、模型名、网络和额度，最后看代码对象和调用方式。

2. 为什么本章要同时关注 `invoke()` 和 `stream()`？

   **参考思路：** `invoke()` 适合验证完整结果，`stream()` 更接近真实产品体验。一个解决"能不能通"，一个解决"用户等待时怎么展示"。

3. 多模型共存时，哪些配置最容易写乱？

   **参考思路：** 模型名、Base URL、API Key、provider 包和默认参数最容易混。应把不同模型的配置集中管理，避免在业务代码里到处散落。

4. 教学 Demo 和真实项目代码最大的差别是什么？

   **参考思路：** Demo 只要跑通，真实项目还要处理配置校验、日志、异常、超时、重试、流式输出和敏感信息隐藏。

**本章小结：**

- **HelloWorld** 的目标是验证整条调用链是否打通。对 LangChain 来说，这条最小链路就是：**准备 API Key、模型名、Base URL → 初始化模型 → `invoke()` 调用 → `.content` 取回复**。
...
- 本章保留的全部案例，分别对应了不同学习目标：`01_GetEnvInfo.py` 用于**检查环境**，`02_LangChainV0.3.py` 与 `03_LangChainV1.0.py` 用于理解 **经典写法与 1.x 写法**，`04_LangChain_MoreV1.0.py` 用于掌握 **多模型共存**，`05_StandardDesc.py` 则让你迈出从"教学 Demo"走向"工程化写法"的第一步。

**建议下一步：** 先亲手跑通本章至少两个脚本，推荐顺序是：`01_GetEnvInfo.py` → `03_LangChainV1.0.py` → `04_LangChain_MoreV1.0.py` → `05_StandardDesc.py`。跑通之后，马上进入 [第 11 章 Model I/O 与模型接入](/stage3-framework/ch11)，把这章中"会用"的部分升级成"真正理解为什么这样接、不同模型如何统一接入"。等你再接着学 [第 13 章 提示词与消息模板](/stage3-framework/ch13) 和 [第 14 章 输出解析器](/stage3-framework/ch14)，就能形成完整的 **输入 → 模型 → 输出** 学习闭环。
