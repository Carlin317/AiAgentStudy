...
**前置知识：** 了解 LangChain 基础（模型调用、Prompt、链式调用）。本章只建立 LangGraph 的第一张图——State、Node、Edge、Graph 四个概念通过最小案例串起来。状态合并详见 [第 23 章](/stage3-framework/ch23)，节点和控制流详见 [第 24 章](/stage3-framework/ch24)，高级能力在 [第 25 章](/stage3-framework/ch25) 和 [第 26 章](/stage3-framework/ch26)。
...
**LangGraph = 用”图 + 状态”来组织和运行 LLM 工作流 / Agent 流程。**
...
LangGraph 不是新的大模型，也不是 Agent 黑盒。它解决的问题是：当 AI 应用需要反复判断下一步、回到前面重试、保存中间状态、让人插手审核时，怎么把流程写得**可控、可维护、可恢复、可观测**。

从工程落地角度看，LangGraph 还把以下生产问题纳入了框架主线：
...
这一章先重点建立“**图 + 状态 + 最小运行方式**”的入门认知；上表里的持久化、流式、人机介入等能力，会在 [第 25 章](/stage3-framework/ch25) 继续展开。

### 1.2 为什么需要 LangGraph

以一个真实场景为例：让 AI 写一份”**小米 SU7 市场分析报告**”，流程不是一次生成就结束，而是：
...
这已不是一条 `prompt | model | parser` 线性链能自然表达的流程了。

![基于 LLM 的智能体工作流示意：开始 → 大模型节点 → 边节点判断下一步 → 结束或执行 Action 并回到大模型](/images/22/22-1-2-1.jpeg)

**Chain / LCEL** 适合固定顺序的线性流水线（”先总结 → 再翻译 → 再解析输出”），但遇到分支、重试、暂停等需求时，容易写出大量 `if / while` 胶水代码。

**Agent** 能自己决定下一步，但执行路径不够稳定，开发者不容易精确约束每一步、也不容易在关键节点插入人工审核或状态恢复。

LangGraph 的价值在于：**保留分支、循环、动态决策的表达能力，同时比纯黑盒 Agent 更容易做流程约束、状态管理、调试追踪和人机协作。**

下面两张图分别从**人机协作**和**多智能体协作**角度说明 LangGraph 的应用。
...
- **LangGraph 可以独立使用，不依赖 LangChain。** 实际项目里节点常调用 LangChain 的模型、工具和 Prompt，所以两者经常搭配出现。
- **LangGraph 不是 Chain 的替代品。** 固定直线流程用 LCEL 更简单；流程出现循环、分支、暂停、恢复时切到 LangGraph。
- **LangChain v1 的 `create_agent` 底层构建在 LangGraph runtime 之上。** 快速搭标准工具调用 Agent 用 `create_agent`；需要显式控制每一步流程和状态时，自己写 LangGraph。
...
- **如果某个字段要“追加”而不是“覆盖”，就要考虑 Reducer。** 本章会通过 `add_messages` 先建立直觉，完整的 Schema / Reducer 机制会在 [第 23 章](/stage3-framework/ch23) 系统讲。

### 1.5 使用场景

不是所有任务都该用 LangGraph。**能用更简单方案稳定解决的问题，不要强行上图编排。**
...
整体印象：**Chain 像一条直线，LangGraph 像一张带状态流转规则的网。**
...
**本教程第 22～26 章主线采用 Graph API。** 先把”状态、节点、边、图”这套显式结构建立起来，后面学习条件边、子图、持久化、多人协作会更顺。Functional API 适合在理解核心思想后再作为补充。

![Quickstart 关键步骤示意：官方提供 Graph API 与 Functional API 两种方式，图中红框突出 Graph API（本教程采用）](/images/22/22-2-1-1.jpeg)

### 2.2 LangGraph 技术架构

LangGraph 的技术架构可以粗略分为三层：
...
**本章和后续第 23～25 章优先学最底层”构建图语法与 API”。** 理解这层之后，才能明白 LangGraph 为什么能做条件跳转、状态持久化、多分支并行和人机中断。

### 2.3 Graph 最小构建流程

LangGraph Graph API 的最小构建流程 5 步：
...
`compile()` 之后得到的 app 才是可运行的图应用对象。编译这一步除了生成可执行对象，还会做基础结构检查，并允许挂接 checkpointer、interrupt 等运行时能力。入门阶段记住”**先搭图，再编译，最后运行**”即可。

### 2.4 环境安装与版本确认

如果你是跟着本仓库完整学习，通常直接在项目根目录执行 `pip install -r requirements.txt` 即可，这样 LangGraph 和前后章节依赖会一起装好，不需要为这一章单独建环境；这个约定和 [第 10 章](/stage3-framework/ch10) 是一致的。
...
- `conda create --name xxx 3.12` 写法不够规范，应写成 `python=3.12`。
- 如果已执行过 `pip install -r requirements.txt`，不需要单独装 `langgraph`；先确认当前终端激活的是正确的项目虚拟环境。

### 2.5 怎么看这张图：ASCII、Mermaid、PNG

LangGraph 比”把流程散落在 `if / while` 里”更容易可视化。编译后通过 `app.get_graph()` 查看图结构，有三种方式：
...
示例代码如下，完整可运行版本见后面的 `02_LangGraphHello.py`（其中已包含 `import uuid`）：
...
第一个案例不接大模型：**输入一个名字，先生成问候语，再追加表情。** 重点不是业务逻辑，而是把 State、Node、Edge、Graph 四个概念串起来。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/02_LangGraphHello.py`

[02_LangGraphHello.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/02_LangGraphHello.py ":include :type=code")

**下图说明**：`draw_mermaid()` 输出粘贴到 Mermaid 在线编辑器后的效果。左侧是 Mermaid 源码，右侧是渲染的流程图。只需看懂 `__start__ → greeting → add_emoji → __end__` 这条执行路径。
...
第二个案例换成业务数据流：**给定初始数值 `x`，先做加法，再做减法，输出结果。** 仍是线性图，但能更清楚地看到状态在节点之间如何传递和改写。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/01_LangGraphBiz.py`

[01_LangGraphBiz.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/01_LangGraphBiz.py ":include :type=code")

### 2.8 案例三：把大模型接进 LangGraph 图里

第三个案例把 LangGraph 和 LangChain 模型调用接起来：**用户输入消息 → model 节点 → 调用聊天模型 → AI 回复写回 messages。**
...
`messages` 字段不是简单覆盖，而是按”追加消息”的规则合并。节点只需返回这一轮新增的 AI 回复，LangGraph 自动追加到已有消息列表后面。对多轮对话、Agent 执行轨迹、人机修正，这种更新方式至关重要。

**这个案例重点看什么：**

- `add_messages` 为什么适合用在对话消息列表上。
- `init_chat_model(...)` 这一段其实复用了 [第 10 章](/stage3-framework/ch10) 的“调用三件套”思维：**模型名、API Key、Base URL**。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/03_LangGraphLLM.py`

[03_LangGraphLLM.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/03_LangGraphLLM.py ":include :type=code")

### 2.9 再往真实项目走一步：并行检索 + 汇总回答为什么很适合用图表达

前面三个案例都是单路线顺序执行。真实项目里，LangGraph 更常见的价值在于**多分支并行执行，再汇总结果**。

例如一个企业问答助手，收到用户问题后同时走两条分支：
...
这段示例帮助建立**项目级图思维**：LangGraph 适合把多个信息来源、多个处理分支、一个最终汇总节点，显式组织成可运行的工作流图。`START` 同时发散到多节点、多上游汇聚到同一下游时状态如何合并，在 [第 23 章](/stage3-framework/ch23) 和 [第 24 章](/stage3-framework/ch24) 展开。
...
1. 什么样的流程值得用 LangGraph 表达，而不是普通函数顺序调用？
   ...
   **参考思路：** 流程有共享状态、分支、循环、并行、暂停恢复或人工介入时适合用。线性脚本够用的任务不必上图。

2. State、Node、Edge 三者如果用业务流程类比，分别是什么？

   **参考思路：** State 是共享工单/上下文，Node 是处理步骤，Edge 是流转规则。图不是画给人看的，而是可执行的流程模型。

3. `compile()` 的意义是什么？为什么它不是可有可无的步骤？

   **参考思路：** 把图描述编译成可运行对象，并做结构校验。没有 compile 就只有描述，没有可执行的应用。

4. 选一个你熟悉的业务流程，把它拆成最少 3 个节点和 1 个条件边。

   **参考思路：** 例如工单处理：分类节点 → 知识库检索节点 → 回复生成节点；条件边根据分类或置信度决定走自动回复还是人工处理。

**本章小结：**

- **LangGraph 是低层图编排框架和运行时**，核心价值是把有状态、可分支、可循环、可暂停、可人工介入的 LLM 流程写得更可控。
...
- **本章三个案例是递进关系**：`02_LangGraphHello.py` 先理解最小图，`01_LangGraphBiz.py` 理解业务状态流，`03_LangGraphLLM.py` 再把聊天模型和 `add_messages` 接进图里。
- 学完本章后应当能：用”**图 + 状态**”概括 LangGraph 核心思想；区分 Graph API 和 Functional API；记住最小落地五步：**定义状态 → 写节点 → 连边 → 编译 → 执行**。

**建议下一步：** 先在本地把 `案例与源码-3-LangGraph框架/01-helloworld` 这 3 个脚本都跑一遍，然后做两个小改造：给 `01_LangGraphBiz.py` 再加一个乘法节点；给 `03_LangGraphLLM.py` 多传一轮历史消息，观察 `messages` 列表是怎么增长的。做完之后，再进入 [第 23 章 - LangGraph API：图与状态](/stage3-framework/ch23)，系统学习 State Schema 和 Reducer。
