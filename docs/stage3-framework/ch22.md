...
[LangGraphHello.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/LangGraphHello.py ":include :type=code")
...
[LangGraphBiz.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/LangGraphBiz.py ":include :type=code")
...
[LangGraphLLM.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/01-helloworld/LangGraphLLM.py ":include :type=code")
...
**建议下一步：** 先在本地把 `案例与源码-3-LangGraph框架/01-helloworld` 这 3 个脚本都跑一遍，然后做两个小改造：给 `LangGraphBiz.py` 再加一个乘法节点；给 `LangGraphLLM.py` 多传一轮历史消息，观察 `messages` 列表是怎么增长的。做完之后，再进入 [第 23 章 - LangGraph API：图与状态](/stage3-framework/ch23)，系统学习 State Schema 和 Reducer。
