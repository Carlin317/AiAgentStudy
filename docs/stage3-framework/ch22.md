...
**学习建议：** 这一章只负责让你建立 LangGraph 的第一张图，不用把后面所有 API 一次学完。先把 State、Node、Edge、Graph 四个词放到一个最小案例里理解：状态怎么进图，节点怎么改状态，边怎么决定下一步。更细的状态合并看 [第 23 章](/stage3-framework/ch23)，节点和控制流看 [第 24 章](/stage3-framework/ch24)，高级能力再放到 [第 25 章](/stage3-framework/ch25) 和 [第 26 章](/stage3-framework/ch26)。
...
至于这里为什么可以从 `START` 同时发散到两个节点、多个上游节点汇聚到同一个下游节点时状态怎么合并，这些更细的 Graph / State 规则，会在 [第 23 章](/stage3-framework/ch23) 和 [第 24 章](/stage3-framework/ch24) 继续展开。
