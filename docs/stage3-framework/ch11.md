...
**前置知识：** 已完成 [第 10 章](/stage3-framework/ch10) 的 HelloWorld 调用。本章从"能调通"升级到"理解调用的结构"，按输入、参数、返回值、接入方式这条线展开。后续 [第 13 章](/stage3-framework/ch13) 和 [第 14 章](/stage3-framework/ch14) 会把 Format 和 Parse 分别拆开讲。

**官方文档与资源**：详见 [工具导航与参考资料索引 - 模型接入与本地运行](/guide/resources#模型接入与本地运行)。
...
**Model I/O** 是 LangChain 中专门负责"**如何和模型打交道**"的模块。它覆盖三件事：**模型输入怎么组织、模型怎么调用、模型输出怎么处理**。

> **Model I/O = Format（输入格式化）→ Predict（模型调用）→ Parse（输出解析）**

这三个词是后续 LangChain 学习里反复出现的主线。
...
- **Format** 主要在 [第 13 章 提示词与消息模板](/stage3-framework/ch13) 展开
- **Predict** 是本章的重点
- **Parse** 主要在 [第 14 章 输出解析器](/stage3-framework/ch14) 展开

### 1.2 为什么需要 Model I/O

学完 HelloWorld 后，很多人会觉得："我已经会 `invoke()` 了，为什么还要单独学这一章？"

**会调一次模型，不等于真的理解了模型接入。**在真实项目里，你迟早会遇到这些问题：
...
这些问题单靠 HelloWorld 讲不透，而它们恰好就是 Model I/O 要解决的事。

### 1.3 生活化类比

Model I/O 就像"点外卖"流程：

- **Format**：下单前把需求写清楚——要不要辣、几人份、送到哪里。写得越清楚，模型越不容易答偏。
- **Predict**：把订单交给餐厅处理。你可以换不同餐厅（不同模型），但下单方式尽量统一。
- **Parse**：外卖到了之后先分装、贴标签。原始回答可能很长，但系统往往需要结构化的数据才能继续处理。

LangChain 做的，本质上就是让"下单方式尽量统一""换餐厅尽量少改代码""拿到结果后更方便继续处理"。

---

## 2、LangChain 模型分类、参数与返回

这一节围绕 Model I/O 中最核心的 **Predict（模型调用）** 展开。需要先弄明白三件事：
...
把这三层分清楚，后面才不会把"换模型""换平台""换 LangChain 写法"混成一件事。
...
后面的 Prompt、Message、Tools、Agent、Memory，本质上大多都建立在 Chat Model 的消息式交互之上。
...
后面在 [第 18 章 向量数据库与 Embedding 实战](/stage3-framework/ch18) 和 [第 19 章 RAG 检索增强生成](/stage3-framework/ch19) 会深入学习它。这里先建立一个概念：**Embedding 也是模型，只是它的输出不是自然语言，而是向量。**
...
大模型在内部会把文本切成 **token** 再计算与生成。token 是比"字"更细或更粗的片段（取决于分词器）：输出往往是**逐个 token 依次生成**。厂商计费、上下文长度上限、接口返回的用量统计，通常也都以 **token 数**为单位。
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/05_ModelIO_Params.py`

[05_ModelIO_Params.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/05_ModelIO_Params.py ":include :type=code")
...
本章先点到为止，后面会在 [第 13 章 提示词与消息模板](/stage3-framework/ch13) 专门展开。现在只需要建立一个概念：

Chat Model 的输入输出，本质上都是"消息（Message）"。

最常见的消息类型包括：`SystemMessage`、`HumanMessage`、`AIMessage`、`ToolMessage`。

从 Model I/O 角度看，**Prompt 只是输入的一种组织方式，Message 才是 Chat Model 更底层的统一表达。**

---

## 3、接入大模型

前面讲的是"调用这一层的通用规律"，这一节进入最实际的部分：**怎么把不同模型接进来。**

### 3.1 三种常见接入思路

在真实项目里，最常见的模型接入方式归纳为三类：
...
某个平台虽然不是 OpenAI，但它把自己的 API 设计成和 OpenAI 接口兼容，因此你可以复用 OpenAI 风格的客户端和调用方式。
...
- 如果只是调一次接口，`openai.OpenAI` 够用
- 如果后面还要接 Prompt、输出解析、LCEL、Agent，通常优先考虑 LangChain 模型对象

#### 3.2.3 案例一：OpenAI 官方 SDK

【案例源码】`案例与源码-2-LangChain框架/02-models_io/04_ModelIO_OpenAI.py`

[04_ModelIO_OpenAI.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/04_ModelIO_OpenAI.py ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/01_ModelIO_ChatOpenAI.py`

[01_ModelIO_ChatOpenAI.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/01_ModelIO_ChatOpenAI.py ":include :type=code")
...
这类写法在旧教程和大量现有项目里普遍使用，你一定要能读懂。

#### 3.2.5 案例三：使用 init_chat_model

【案例源码】`案例与源码-2-LangChain框架/02-models_io/03_ModelIO_Init_chat_model.py`

[03_ModelIO_Init_chat_model.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/03_ModelIO_Init_chat_model.py ":include :type=code")

这就是当前更推荐掌握的 1.x 统一入口写法。
...
本课程主线更偏向 `init_chat_model`，但不会忽略 `ChatOpenAI`，因为后者在真实项目和旧资料里仍然普遍使用。
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/02_ModelIO_DeepSeek.py`

[02_ModelIO_DeepSeek.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/02_ModelIO_DeepSeek.py ":include :type=code")
...
从"统一风格、少记类名"出发，本课程更推荐优先理解第一种；从"只用通义、希望原生接入"出发，也要认识第二种。
...
这也是你在 [第 10 章 HelloWorld](/stage3-framework/ch10) 里已经接触过的主线写法。
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/06_ModelIO_Qwen.py`

[06_ModelIO_Qwen.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-2-LangChain框架/02-models_io/06_ModelIO_Qwen.py ":include :type=code")
...
智谱是一个独立 provider。在 LangChain 中接它时，通常不是走阿里百炼那种 OpenAI 兼容路线，而是走它自己的集成方式。
...
- 也有不少 provider 走**原生集成**路线
- 换厂商时，核心思维还是一样：**安装对应 provider 包 → 配 Key → 初始化模型对象 → `invoke()`**

### 3.6 真实项目的选型建议

从企业项目或个人长期项目的角度出发，给一个稳妥的建议：
...
1. `Format -> Predict -> Parse` 这条线里，哪一步最容易被初学者忽略？后果是什么？
   ...
   **参考思路：** 很多人只盯着 Predict，忽略输入格式和输出解析。结果就是前面喂得乱、后面接不住，系统看似调通了，实际很难接业务。

2. "模型能力""服务提供商""LangChain 模型对象"混在一起，会导致什么设计问题？

   **参考思路：** 会把能力、访问方式和代码封装混淆。分开后才方便换模型、换供应商或换封装。

3. 为什么聊天模型返回 `AIMessage` 比返回字符串更适合工程链路？

   **参考思路：** 真实返回里不仅有文本，还有角色、工具调用、metadata、token 用量和停止原因。保留结构化消息，后续工具调用、日志和解析更稳。

4. 接入一个新模型平台时，你会优先试统一入口还是 provider 原生类？

   **参考思路：** 标准聊天调用用统一入口更省心；需要供应商特有参数或已绑定 provider 类时，原生封装更合适。判断标准是需求，不是写法新旧。
...
- 在实际项目中，Chat Model 是最常用的模型类型。它比传统 LLM 更适合多轮对话、消息结构、工具调用和 Agent 场景，返回结果通常是 **`AIMessage`**，最常用字段是 `content`、`response_metadata` 和 `usage_metadata`。

**建议下一步：** 先把本章的 `02-models_io` 目录按顺序跑一遍，推荐顺序是：`04_ModelIO_OpenAI.py` → `01_ModelIO_ChatOpenAI.py` → `03_ModelIO_Init_chat_model.py` → `05_ModelIO_Params.py` → `02_ModelIO_DeepSeek.py` → `06_ModelIO_Qwen.py`。跑完之后，再进入 [第 12 章 Ollama 本地部署与调用](/stage3-framework/ch12)、[第 13 章 提示词与消息模板](/stage3-framework/ch13)、[第 14 章 输出解析器](/stage3-framework/ch14)，把本章中的 "模型接入" 和后续的 "输入组织""输出解析" 串成完整闭环。
