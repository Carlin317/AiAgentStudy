...
**建议下一步：** 先把无记忆、内存版、多 session 版、Redis 版案例都跑一遍，重点观察“同一个 `session_id` 下历史是怎么被延续的”；然后继续学习 [第 17 章 Tools 工具调用](/stage3-framework/ch17)，你会更容易理解为什么 Agent 场景不仅需要工具，还需要会话状态与历史管理。
