...
**学习建议：** MCP 先别当成一堆协议名背，它的核心是把外部能力接入模型应用时变得更标准。第一遍只抓三组关系：Tools / Resources / Prompts 是暴露的能力，Host / Client / Server 是通信角色，stdio / Streamable HTTP 是传输方式。学完 [第 21 章 Agent 智能体](/stage3-framework/ch21) 后再回来看 `Agent + MCP`，会更容易理解它为什么重要。
...
> **术语约定：** 本章里表格写作 **Tool / Function Calling**，是为了兼容不同文档语境；和 [第 17 章](/stage3-framework/ch17) 一样，你可以把它们先看成同一层“模型如何表达工具调用意图”的机制。
