...
- 能运行并理解本章全部案例，为后续 [第 24 章 LangGraph API：节点、边与进阶](/stage3-framework/ch24) 打基础。

**前置知识：** 需要了解 [第 22 章](/stage3-framework/ch22) 的 State、Node、Edge、Graph 四个概念。本章重点是状态如何被安全地更新——看每个案例时关注三点：节点读了 State 里的什么，返回了哪些局部更新，Reducer 怎么合并。
...
[第 22 章](/stage3-framework/ch22) 已经讲过 LangGraph 四要素了，那为什么这一章还要把 Graph 单独拎出来？原因是：**上一章主要帮你建立“图能跑起来”的直觉，这一章要开始把“图构建器、状态接口、状态合并规则”讲得更工程化。**
...
如果先不看条件边、`Send`、`Command`、`Runtime` 等进阶能力（这些在 [第 24 章](/stage3-framework/ch24) 系统讲），构建一张最基础的 `StateGraph`，最核心的流程还是下面 5 步：
...
【案例源码】`案例与源码-3-LangGraph框架/02-graph/01_BuildWholeGraphSummary.py`

[01_BuildWholeGraphSummary.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/02-graph/01_BuildWholeGraphSummary.py ":include :type=code")
...
核心公式：**State = Schema（模式） + Reducer（规约函数）**
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/01_DefState.py`

[01_DefState.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/01_DefState.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/schema/01_StateSchema.py`

[01_StateSchema.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/schema/01_StateSchema.py ":include :type=code")
...
如果一个字段没有写 `Annotated[..., reducer]`，默认就是**覆盖更新**。这种行为适合”只关心最新值”的场景，比如 `current_step`、`latest_answer`、`final_summary`。

这个案例里，`node1` 先把 `foo` 改成 `22`，`node2` 再把 `bar` 改成新列表。因为 `foo`、`bar` 都没声明特殊 Reducer，所以它们都是按默认覆盖规则更新。

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/03_StateReducer_Default.py`

[03_StateReducer_Default.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/03_StateReducer_Default.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/01_StateReducer_AddMessages.py`

[01_StateReducer_AddMessages.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/01_StateReducer_AddMessages.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/04_StateReducer_OperatorAdd.py`

[04_StateReducer_OperatorAdd.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/04_StateReducer_OperatorAdd.py ":include :type=code")

**字符串连接案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/05_StateReducer_OperatorAdd2.py`

[05_StateReducer_OperatorAdd2.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/05_StateReducer_OperatorAdd2.py ":include :type=code")

**数值累加案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/06_StateReducer_OperatorAdd3.py`

[06_StateReducer_OperatorAdd3.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/06_StateReducer_OperatorAdd3.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/07_StateReducer_OperatorMul.py`

[07_StateReducer_OperatorMul.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/07_StateReducer_OperatorMul.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/02_StateReducer_Custom.py`

[02_StateReducer_Custom.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/02_StateReducer_Custom.py ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/08_StateReducersMyChatBot.py`

[08_StateReducersMyChatBot.py](https://github.com/didilili/ai-agents-from-zero/blob/main/案例与源码-3-LangGraph框架/03-state/reducers/08_StateReducersMyChatBot.py ":include :type=code")
...
**建议下一步：** 先按顺序跑一遍 `01_BuildWholeGraphSummary.py`、`01_DefState.py`、`01_StateSchema.py` 和 `03-state/reducers` 目录下所有 Reducer 案例，然后自己做两个小练习：把 `01_BuildWholeGraphSummary.py` 的 `process_data` 改成用 `operator.add` 追加历史记录；把 `08_StateReducersMyChatBot.py` 再加一个 `latest_summary` 字段，用默认覆盖保存最新摘要。做完后进入 [第 24 章 - LangGraph API：节点、边与进阶](/stage3-framework/ch24)，继续学习节点、普通边、条件边和 `Command` / `Send` / `Runtime`。
