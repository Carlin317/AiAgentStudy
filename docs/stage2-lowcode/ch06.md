...
如果你对镜像、容器、Compose、volume、端口映射这些概念还不熟，可以先看 [第 8 章 Docker 入门与 Dify 部署常见问题](/stage2-lowcode/ch08)。本章只保留部署必需步骤。
...
本章不展开 Docker 网络排障，遇到问题时优先参考：[第 8 章 - 网络慢或镜像拉取失败](/stage2-lowcode/ch08#12、网络慢或镜像拉取失败)。
...
这条命令会通过 Compose 启用所有 profile，并在后台启动服务。`docker compose up -d` 的通用含义见 [第 8 章](/stage2-lowcode/ch08#3、compose-到底做了什么)。
...
本节只讲最小部署流程。Docker 通用概念、镜像拉取失败、容器状态异常、数据位置、备份升级和数据库连接，统一放在 [第 8 章 Docker 入门与 Dify 部署常见问题](/stage2-lowcode/ch08)。
...
可以把 `.env` 理解为这套部署环境的运行参数表。Docker / Compose / 数据卷的关系，以及 Dify 场景下的排障方式，见 [第 8 章](/stage2-lowcode/ch08)。
...
本章只负责跑通最小链路。更系统的 Docker 命令、Dify 数据位置、升级备份和数据库连接，继续看 [第 8 章 Docker 入门与 Dify 部署常见问题](/stage2-lowcode/ch08)。
...
**建议下一步：** 如果你要继续看 Dify 部署后的 Docker 概念、数据位置、升级备份和排障，进入 [第 8 章 Docker 入门与 Dify 部署常见问题](/stage2-lowcode/ch08)；如果你要继续看更完整的企业部署链路，再进入 [第 7 章 企业级大模型部署](/stage2-lowcode/ch07)。
